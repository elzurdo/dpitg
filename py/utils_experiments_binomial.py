...
def successes_failures_caculate_hdi_limits(successes, failures, ci_fraction=CI_FRACTION):
    aa = int(successes)
    bb = int(failures)
    ...
    hdi_min, hdi_max = successes_failures_to_hdi_ci_limits(aa, bb, ci_fraction=ci_fraction)
...
class BinaryAccounting():
    ...
    def successes_failures_to_hdi_limits(self, successes, failures):
        ...
        if pair not in self.dict_successes_failures_hdi_limits:
            self.dict_successes_failures_hdi_limits[pair] =\
                successes_failures_caculate_hdi_limits(successes, failures, ci_fraction=self.ci_fraction)
    ...
    def save(self):
        path = self.filepath
        entries_new = len(self.dict_successes_failures_hdi_limits)
        entries_old = 0
        if path.exists():
            with open(path, "rb") as f:
                data = pickle.load(f)
            entries_old = len(data["hdi_limits"])
        ...
        payload = {"version": 1, "hdi_limits": self.dict_successes_failures_hdi_limits}
